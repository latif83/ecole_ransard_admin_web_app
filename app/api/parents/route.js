...
export async function GET() {
    try {
        const parents = await prisma.Parents.findMany({
            include: {
                students: true, // Include the students related to each parent
            },
        });
...
export async function POST(req) {
    try {
        const { firstName, lastName, email, phone, address, students } = await req.json();
        ...
        return new NextResponse(JSON.stringify({ message: "Parent created successfully", }), { status: 201 });
    } catch (error) {
...
export async function PUT(req) {
    try {
        const { id, firstName, lastName, email, phone, address, students } = await req.json();
